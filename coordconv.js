// https://github.com/AngLaboratory/coordconv/blob/main/coordconv.js 코드를 esm 으로 바꾸고 this -> COORD_CODE 로 리팩토링
...
let coordconv = function (x, y, fromType, toType) {
    let vCoordConv = {
        init : (x, y) => {
            COORD_CODE.COORD_TM         = 0
            COORD_CODE.COORD_WTM        = 1
            COORD_CODE.COORD_CONGNAMUL  = 2
            COORD_CODE.COORD_WCONGNAMUL = 3
            COORD_CODE.COORD_KTM        = 4
            COORD_CODE.COORD_WKTM       = 5
            COORD_CODE.COORD_UTM        = 6
            COORD_CODE.COORD_WGS84      = 7
            COORD_CODE.COORD_BESSEL     = 8

            COORD_CODE.COORD_BASE = []
            COORD_CODE.COORD_BASE[COORD_CODE.COORD_TM        ] = { name : "TM"        , code : COORD_CODE.COORD_TM        }
            COORD_CODE.COORD_BASE[COORD_CODE.COORD_WTM       ] = { name : "WTM"       , code : COORD_CODE.COORD_WTM       }
            COORD_CODE.COORD_BASE[COORD_CODE.COORD_CONGNAMUL ] = { name : "CONGNAMUL" , code : COORD_CODE.COORD_CONGNAMUL }
            COORD_CODE.COORD_BASE[COORD_CODE.COORD_WCONGNAMUL] = { name : "WCONGNAMUL", code : COORD_CODE.COORD_WCONGNAMUL}
            COORD_CODE.COORD_BASE[COORD_CODE.COORD_KTM       ] = { name : "KTM"       , code : COORD_CODE.COORD_KTM       }
            COORD_CODE.COORD_BASE[COORD_CODE.COORD_WKTM      ] = { name : "WKTM"      , code : COORD_CODE.COORD_WKTM      }
            COORD_CODE.COORD_BASE[COORD_CODE.COORD_UTM       ] = { name : "UTM"       , code : COORD_CODE.COORD_UTM       }
            COORD_CODE.COORD_BASE[COORD_CODE.COORD_WGS84     ] = { name : "WGS84"     , code : COORD_CODE.COORD_WGS84     }
            COORD_CODE.COORD_BASE[COORD_CODE.COORD_BESSEL    ] = { name : "BESSEL"    , code : COORD_CODE.COORD_BESSEL    }

            COORD_CODE.a6377397 = [6377397.155, 0.0033427731799399794]
            COORD_CODE.a6378137 = [6378137    , 0.0033528106647474805]

            COORD_CODE.g51 = [500000, 200000, 1     , 38, 127.0028902777777777776]
            COORD_CODE.g52 = [500000, 200000, 1     , 38, 127]
            COORD_CODE.g6  = [600000, 400000, 0.9999, 38, 128]
            COORD_CODE.g0  = [     0, 500000, 0.9996,  0, 129]
        },

        getTransCoord : (xy, fromType, toType) => {
            if ( fromType == toType ) return xy
            vCoordConv.init(x, y)
            if ( fromType < 0 || fromType >= COORD_CODE.COORD_BASE.length ) return xy
            if ( toType   < 0 || toType   >= COORD_CODE.COORD_BASE.length ) return xy
        ...
        from2bassel : (xy, fromType) => {
            if ( fromType == COORD_CODE.COORD_TM ) {
                return vCoordConv.convertTM2BESSEL(xy);
            } else if ( fromType == COORD_CODE.COORD_WTM ) {
                let cxy = vCoordConv.convertWTM2WGS(xy);
                return vCoordConv.convertWGS2BESSEL(cxy);
            } else if ( fromType == COORD_CODE.COORD_CONGNAMUL ) {
                return vCoordConv.convertCONG2BESSEL(xy);
            } else if ( fromType == COORD_CODE.COORD_WCONGNAMUL ) {
                let cxy = vCoordConv.convertWCONG2WGS(xy);
                return vCoordConv.convertWGS2BESSEL(cxy);
            } else if ( fromType == COORD_CODE.COORD_KTM ) {
                return vCoordConv.convertKTM2BESSEL(xy);
            } else if ( fromType == COORD_CODE.COORD_WKTM ) {
                let cxy = vCoordConv.convertWKTM2WGS(xy);
                return vCoordConv.convertWGS2BESSEL(cxy);
            } else if ( fromType == COORD_CODE.COORD_UTM ) {
                let cxy = vCoordConv.convertUTM2WGS(xy);
                return vCoordConv.convertWGS2BESSEL(cxy);
            } else if ( fromType == COORD_CODE.COORD_WGS84 ) {
                return vCoordConv.convertWGS2BESSEL(xy);
            } else if ( fromType == COORD_CODE.COORD_BESSEL ) {
                return xy
            }
        },
        bassel2to : (xy, toType) => {
            if ( toType == COORD_CODE.COORD_TM ) {
                return vCoordConv.convertBESSEL2TM(xy);
            } else if ( toType == COORD_CODE.COORD_WTM ) {
                let cxy = vCoordConv.convertBESSEL2WGS(xy);
                return vCoordConv.convertWGS2WTM(cxy);
            } else if ( toType == COORD_CODE.COORD_CONGNAMUL ) {
                return vCoordConv.convertBESSEL2CONG(xy);
            } else if ( toType == COORD_CODE.COORD_WCONGNAMUL ) {
                let cxy = vCoordConv.convertBESSEL2WGS(xy);
                return vCoordConv.convertWGS2WCONG(cxy);
            } else if ( toType == COORD_CODE.COORD_KTM ) {
                return vCoordConv.convertBESSEL2KTM(xy);
            } else if ( toType == COORD_CODE.COORD_WKTM ) {
                let cxy = vCoordConv.convertBESSEL2WGS(xy);
                return vCoordConv.convertWGS2WKTM(cxy);
            } else if ( toType == COORD_CODE.COORD_UTM ) {
                let cxy = vCoordConv.convertBESSEL2WGS(xy);
                return vCoordConv.convertWGS2UTM(cxy);
            } else if ( toType == COORD_CODE.COORD_WGS84 ) {
                return vCoordConv.convertBESSEL2WGS(xy);
            } else if ( toType == COORD_CODE.COORD_BESSEL ) {
                return xy
            }
        },
        convertTM2BESSEL : xy => {
            return vCoordConv.TM2GP(xy, COORD_CODE.a6377397, COORD_CODE.g51);
        },
        convertBESSEL2TM : xy => {
            return vCoordConv.GP2TM(xy, COORD_CODE.a6377397, COORD_CODE.g51);
        },
        convertWTM2WGS : xy => { 
            return vCoordConv.TM2GP(xy, COORD_CODE.a6378137, COORD_CODE.g52);
        },
        convertWGS2WTM : xy => {
            return vCoordConv.GP2TM(xy, COORD_CODE.a6378137, COORD_CODE.g52);
        },
        convertCONG2BESSEL : xy => {
            let cxy = vCoordConv.shiftIsland(xy, false);
            return vCoordConv.TM2GP(cxy, COORD_CODE.a6377397, COORD_CODE.g51);
        },
        convertBESSEL2CONG : xy => {
            let cxy = vCoordConv.GP2TM(xy, COORD_CODE.a6377397, COORD_CODE.g51);
            return vCoordConv.shiftIsland(cxy, true);
        },
        convertWCONG2WGS : xy => {
            let cxy = [ xy[0] / 2.5, xy[1] / 2.5 ];
            return vCoordConv.TM2GP(cxy, COORD_CODE.a6378137, COORD_CODE.g52);
        },
        convertWGS2WCONG : xy => {
            let cxy = vCoordConv.GP2TM(xy, COORD_CODE.a6378137, COORD_CODE.g52);
            return [ Math.round(cxy[0] * 2.5), Math.round(cxy[1] * 2.5) ];
        },
        convertKTM2BESSEL : xy => {
            return vCoordConv.TM2GP(xy, COORD_CODE.a6377397, COORD_CODE.g6);
        },
        convertBESSEL2KTM : xy => {
            return vCoordConv.GP2TM(xy, COORD_CODE.a6377397, COORD_CODE.g6);
        },
        convertWKTM2WGS : xy => {
            return vCoordConv.TM2GP(xy, COORD_CODE.a6378137, COORD_CODE.g6);
        },
        convertWGS2WKTM : xy => {
            return vCoordConv.GP2TM(xy, COORD_CODE.a6378137, COORD_CODE.g6);
        },
        convertUTM2WGS : xy => {
            return vCoordConv.TM2GP(xy, COORD_CODE.a6378137, COORD_CODE.g0);
        },
        convertWGS2UTM : xy => {
            return vCoordConv.GP2TM(xy, COORD_CODE.a6378137, COORD_CODE.g0);
        },
        convertBESSEL2WGS : xy => {
            let rtn = vCoordConv.GP2CTR(xy, COORD_CODE.a6377397);
            rtn = vCoordConv.InverseMolod(rtn[0], rtn[1], rtn[2]);
            return vCoordConv.CTR2GP(rtn[0], rtn[1], rtn[2], COORD_CODE.a6378137);
        },
        convertWGS2BESSEL : xy => {
            let rtn = vCoordConv.GP2CTR(xy, COORD_CODE.a6378137);
            rtn = vCoordConv.TransMolod(rtn[0], rtn[1], rtn[2]);
            return vCoordConv.CTR2GP(rtn[0], rtn[1], rtn[2], COORD_CODE.a6377397);
        },
