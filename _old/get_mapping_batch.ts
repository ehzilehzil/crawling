...
async function main() {
    ...
    while (true) {
        ...
        const docs = await nice.find({ $or: [{"status.state": "notfound"}, {"status.state": {$exists: false}}]}).limit(500).toArray();
        ...
        for (const nice_doc of docs) {
            ...
            const naver_doc = naver.find({
                longitude: { $gte: nice_x - 0.002, $lte: nice_x + 0.002 },
                latitude: { $gte: nice_y - 0.002, $lte: nice_y + 0.002 },
            });
...
function j(x: string, y: string): number {
    const x_a = x.replace(/[^가-힣]/g, "").toLowerCase();
    const y_a = y.replace(/[^가-힣]/g, "").toLowerCase();
